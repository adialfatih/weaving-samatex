...
<body>
    ...
    <div class="container">
        ...
        <a style="background:green;color:#FFFFFF;width:105px;display:flex;justify-content:center;align-items:center;margin-top:15px;font-size:12px;padding:5px 0;border-radius:7px;margin-left:5px;" id="idsimpanpkg2" >Simpan</a>
        </div>
        <br>
    ...
    <script>
        ...
        function delpkg(kode,kode2){
            //peringatan('del'+kode);
            var conf = confirm('Anda yakin ingin menghapus kode '+kode2+'?');
            if (conf == true) {
                $.ajax({
        ...
        function peringatan(txt) {
            ...
            loadisi();
            $( "#idsimpanpkg2" ).on( "click", function() {
                $.ajax({
                    url:"<?=base_url();?>users2/simpanpkgoke",
                    type: "POST",
                    data: {"pkg" : pkg},
                    cache: false,
                    success: function(dataResult){
                        var dataResult = JSON.parse(dataResult);
                        if(dataResult.statusCode==200){
                            suksestoast('Berhasil menyimpan validasi');
                            setTimeout(() => {
            ...
            function validasi(id){
                $.ajax({
                    url:"<?=base_url();?>users2/validasi_isipaket",
                    type: "POST",
                    data: {"id" : id},
                    cache: false,
                    success: function(dataResult){
