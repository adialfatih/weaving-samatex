            ...
            <div class="fortable has" style="font-size: 13px;min-height: calc(100vh - 200px);">
                <table id="idtable_owek">
                    ...
                    foreach ($qry as $row) {
                        ...
                        $jmlRoll = $this->db->query("SELECT * FROM data_fol WHERE konstruksi='$konstruksi' AND jns_fold='Grey' AND ( posisi='Samatex' OR posisi LIKE '%PKT%' )")->num_rows();
                        $totalPanjang = $this->db->query("SELECT SUM(ukuran) AS ukr FROM data_fol WHERE konstruksi='$konstruksi' AND jns_fold='Grey' AND ( posisi='Samatex' OR posisi LIKE '%PKT%' )")->row("ukr");
                        if($jmlRoll > 0){
                        echo "<tr>";
                        echo "<td>".$no++."</td>";
                        ...
                        echo "</tr>";
                        $this->data_model->updatedata('kode_konstruksi',$konstruksi,'data_stok',['prod_fg'=>$totalPanjang]);
                        }
                    }
                    ?>
                </table>
