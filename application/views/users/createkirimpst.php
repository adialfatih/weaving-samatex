...
<body>
    ...
    <div class="container">
        <div class="form-label">
            <small>Ketik dan pilih kode roll untuk menambahkan ke dalam paket TES</small>
        </div>
        <div class="form-label">
        ...
        <a style="background:green;color:#FFFFFF;width:105px;display:flex;justify-content:center;align-items:center;margin-top:15px;font-size:12px;padding:5px 0;border-radius:7px;margin-left:5px;" href="<?=base_url('Usersporses/savepkgpermanent/'.$kd);?>">Simpan</a>
        </div>
        <br>
    ...
    <?php
        $kdrol = $this->db->query("SELECT kode_roll FROM data_ig WHERE konstruksi='$kons' AND loc_now IN( 'Samatex','fg','FG') AND kode_roll NOT IN (SELECT kode_roll FROM data_if) AND kode_roll NOT IN (SELECT kode_roll FROM data_fol)");
