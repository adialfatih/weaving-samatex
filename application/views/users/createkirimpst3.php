...
<body>
    ...
    <div class="container">
        ...
        <form action="<?=base_url('Usersporses/returs');?>" method="post">
        <button stype="submit" style="outline:none;border:none;background:green;color:#FFFFFF;width:105px;display:flex;justify-content:center;align-items:center;margin-top:15px;font-size:12px;padding:5px 0;border-radius:7px;margin-left:5px;">Simpan</button>
        </div>
        <input type="hidden" id="detil" name="detil">
        </form>
    </div>
    ...
    <script>
        ...
        function peringatan(txt) {
            ...
            function returRjs(KD){
                ...
                //     }
                // });
                var dataInput = document.getElementById('detil');
                var value = dataInput.value.split(',').map(item => item.trim());

                var index = value.indexOf(KD);
                if (index !== -1) {
                    // KD exists, remove it
                    value.splice(index, 1);
                } else {
                    // KD does not exist, add it
                    value.push(KD);
                }

                // Update the input value
                dataInput.value = value.join(', ');
            }
