...
<body>
    ...
    <script>
        ...
        document.getElementById("autoComplete").addEventListener("change", function() {
            ...
            const apiUrl = `https://inspect.rdgjt.com/api/inspect?id=${id}`;

            let savedCodes = new Set(); // Inisialisasi di luar agar bisa diakses di fetch kedua

            fetch("https://sm.rdgjt.com/api/savedCodes")
                .then(response => response.json())
                .then(savedData => {
                    if (savedData.status && Array.isArray(savedData.data)) {
                        savedCodes = new Set(savedData.data.map(item => item.kode_roll)); // Pastikan data dalam bentuk Set
                        //console.log("Daftar kode yang sudah ada:", savedCodes);
                    } else {
                        console.error("Data tidak valid atau kosong:", savedData);
                    }

                    return fetch(apiUrl); // Kembalikan fetch kedua agar tetap dalam chaining promise
                })
                .then(response => response.json())
                .then(data => {
                    if (data.status && data.jumlahData > 0) {
                        ...
                        data.data.forEach(item => {
                            const isChecked = savedCodes.has(item.kode) ? "checked" : ""; // Perbaikan: savedCodes sudah dideklarasikan
                            outputHtml += `
                                <tr>
                                    <td>${item.kode}</td>
                                    <td>${item.ukuran}</td>
                                    <td>${item.konstruksi}</td>
                                    <td><input type="checkbox" onclick="addPkg('${item.kode}','${item.kd}',this.checked)" ${isChecked}></td>
                                </tr>
                            `;
                        });
