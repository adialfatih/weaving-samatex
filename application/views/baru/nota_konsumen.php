						...
						<div class="pd-20">
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                                $ad_total_tagihan+= floatval($ar_data3[$idx]);
                            ...
                            <textarea class="form-control" style="width:400px;height:70px;" name="ket123" placeholder="Tambahkan keterangan jika perlu. Atau bisa anda abaikan"></textarea>
                            <br>
                            <button type="submit" class="btn btn-success">Bayar</button>
                            </form>
                            <?php //echo "<hr>".number_format($ad_total_tagihan);
                            $all_bayar = 0;
                            foreach($ar_idcus as $aridcus){
                                $cek_bayar6 = $this->db->query("SELECT SUM(nominal_pemb) as nilai FROM a_nota_bayar2 WHERE id_cus='$aridcus'")->row("nilai");
                                if($cek_bayar6 > 0){
                                    $all_bayar+= floatval($cek_bayar6);
                                }
                                //echo "<br>".$cek_bayar6."-".$aridcus."-tes";
                            }
                            //echo "tes-weo-".$all_bayar;
                            $asli_kurange_piro = $ad_total_tagihan - $all_bayar;
                            if($customer['nama_konsumen']=="IBAD"){
                                $showpiutang = $asli_kurange_piro + 1404900;
                            } else {
                                $showpiutang = $asli_kurange_piro;
                            }
                            //echo "===".$asli_kurange_piro;
                            if(fmod($asli_kurange_piro, 1) !== 0.00){
                                $asli_kurange_piro2 = number_format($showpiutang,2,',','.');
                            } else {
                                $asli_kurange_piro2 = number_format($showpiutang,0,',','.');
                            }
                            
                            echo "<script>";
                            echo "document.getElementById('totalpiutang').innerHTML = 'Rp. ".$asli_kurange_piro2."';";
                            echo "</script>";
                            ?>
						</div>
					</div>
